import { IDoesFilterPassParams, IFilter, RowNode } from '@ag-grid-community/all-modules';
import { isEqual, orderBy, uniqBy } from 'lodash';
import React from 'react';

import { ISelectionFilter, ISelectionFilterParams, SelectableItem, SelectionFilterModel, State } from './interfaces';
...
export class SelectionFilter extends React.Component<ISelectionFilterParams, State> implements IFilter {
  public constructor(props: ISelectionFilterParams) {
    ...
    this.disableOrderBy = props.disableOrderBy || false;

    // order the selected items and make sure they are unique.
    if (this.disableOrderBy) {
      this.selectableItems = uniqBy<SelectableItem>(props.selectableItems, 'name');
    } else {
      this.selectableItems = orderBy<SelectableItem>(uniqBy<SelectableItem>(props.selectableItems, 'name'), 'name');
    }
  }

  private readonly valueGetter: (rowNode: RowNode) => any;
  private readonly selectableItems: SelectableItem[];
  private readonly multiple: boolean;
  private readonly disableOrderBy: boolean;
  ...
  public doesFilterPass(params: IDoesFilterPassParams) {
    return (
      ...
      !!this.state.selectedItems.find(selectedItem => isEqual(selectedItem, this.valueGetter(params.node)))
    );
  }

  public getModel(): SelectionFilterModel | undefined {
  ...
  public setModel(model: SelectionFilterModel) {
    let newState: State;
    ...
    if (event.target.checked) {
      ...
      if (this.multiple) {
        if (newItem.exclusive) {
          newItems = [newItem];
        } else if (newItem && !this.state.selectedItems.some(item => item.name === selectedName)) {
          newItems = [...newItems.filter(item => !item.exclusive), newItem];
        }
      } else if (newItem) {
        newItems = [newItem];
      }
...
export function getSelectionFilter(list: string[], multiple = false): ISelectionFilter {
  return {
    filter: 'selectionFilter',
    filterParams: {
      multiple,
      selectableItems: list.map(item => ({ name: item, value: item })),
    },
  };
}
